...
model = AutoModelForSequenceClassification.from_pretrained("google/muril-base-cased", num_labels=classes)
...
unfreeze_layer = int(sys.argv[1])

for layer_no in range(0,layers-unfreeze_layer):
    print("Freezing layer : ", layer_no)
        ...
        if i % 100 == 0:    # print every 2000 mini-batches
            ...
            with torch.no_grad():
                ...
                if curr_val_acc > max_val_acc:
                    if max_val_acc:
                        os.remove('../result_unfreeze_'+str(unfreeze_layer)+'/basline_nn_simple.pt')
                    torch.save(model, '../result_unfreeze_'+str(unfreeze_layer)+'/basline_nn_simple.pt')

                    # model_scripted = torch.jit.script(model) # Export to TorchScript
                    # model_scripted.save('../result_unfreeze_'+str(unfreeze_layer)+'/basline_nn_jit.pt')
                    max_val_acc = curr_val_acc

            # print(f'Accuracy of the network on the {total} valid inputs: {100 * correct / total} ')
            file_log = open('../result_unfreeze_'+str(unfreeze_layer)+'/acc_log.txt', 'a')
...
# torch.save(net, '../result_unfreeze_'+str(unfreeze_layer)+'/basline_nn_simple.pt')
