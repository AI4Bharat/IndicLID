...
file = open('filter_lang_stats.txt', 'w')
file.write( 'lang_code' + '\t' + 'length_roman_sen_non_valid' + '\t' + 'length_roman_sen_valid' + '\t' + 'length_roman_dakshina_filter_list' + '\t' + 'length_roman_sen_valid_minus_dakshina_filter_list'+ '\t' + 'length_final_merge_list' + '\n')


for lang_code in lang_code_list:

    # validated roman inputs
    df = pd.read_excel(final_merge_xls, lang_code + '_scored')
    dakshina_filter_list = df.values.tolist()
    # print(dakshina_filter_list[0:10])
    roman_dakshina_filter_list = [line[2] for line in dakshina_filter_list if line[1] == 0 ]
    # print(roman_dakshina_filter_list[0])
    ...
    roman_sen_non_valid = [line[1] for line in lines_non_validation]
    # print(roman_sen_non_valid[0])

    roman_final_filter_list = roman_dakshina_filter_list + roman_sen_non_valid
    ...
    roman_sen_valid = [line[1] for line in lines_validation]
    # print(roman_sen_valid[0])

    # reverificaiton
    set_difference = set(roman_sen_valid).difference(set(roman_dakshina_filter_list))
    
    file.write( lang_code + '\t' + str(len(roman_sen_non_valid)) + '\t' + str(len(roman_sen_valid)) + '\t' + str(len(roman_dakshina_filter_list)) + '\t' + str(len(set_difference))+ '\t' + str(len(roman_final_filter_list)) + '\n')

    # print(len(set_difference))
    # print(set_difference)
    
    file_dakshina_filter = open('final_merge_set/'+lang_code+'_filter.txt', 'w')
    file_dakshina_filter.write('\n'.join(roman_final_filter_list))
