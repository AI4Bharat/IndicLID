...
MuRIL_lid_model_path = '/nlsasfs/home/ai4bharat/yashkm/yash/indic-lid/final_runs/roman_model/finetuning/clean_samples/MuRIL/unfreeze_layers/result_unfreeze_2/basline_nn_simple.pt'
model = torch.load(MuRIL_lid_model_path)
model.eval()
            ...
            for sen, label, pred_label, logit in zip(inputs, labels, predicted, outputs.logits):
                ...
                if fasttext_pred_score > threshold:
                    ...
                    if label.item() == pred_label.item():
                        final_count+=1
                        csv_writer_predictions_right.writerow( [ sen, confusion_matrix_reverse_mapping[label.item()], confusion_matrix_reverse_mapping[pred_label.item()], pred_score, 'MuRIL' ] )
                        confusion_matrix[ label.item() ][ pred_label.item() ] += 1

                    else:
                        csv_writer_predictions_wrong.writerow( [ sen, confusion_matrix_reverse_mapping[label.item()], confusion_matrix_reverse_mapping[pred_label.item()], pred_score, 'MuRIL' ] )
                        csv_writer_predictions_wrong.writerow( [ sen, confusion_matrix_reverse_mapping[label.item()], fasttext_pred_label[9:], fasttext_pred_score, 'fasttext' ] )
                        confusion_matrix[ label.item() ][ pred_label.item() ] += 1
...
evaluate('test_combine_roman', 20)
evaluate('test_combine_romanized_indicxlit', 20)

evaluate('test_benchmark_roman', 9)
evaluate('test_benchmark_romanized_indicxlit', 9)

evaluate('test_dakshina_roman', 11)
evaluate('test_dakshina_romanized_indicxlit', 11)
