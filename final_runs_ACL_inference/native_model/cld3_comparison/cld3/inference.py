...
def evaluate(test_file_name, languages):
    ...
    for line in lines_test:
        ...
        if pred_label in cld3_reverse_dict:
            pred_label = '__label__'+cld3_reverse_dict[pred_label]
        else:
        ...
        if pred_label == label:
            count+=1
            csv_writer_predictions_right.writerow( [ sen, label, pred_label, 0 ] )
        else:
            csv_writer_predictions_wrong.writerow( [ sen, label, pred_label, 0 ] )
        n+=1   

        confusion_matrix[ confusion_matrix_mapping[label[9:]] ][ confusion_matrix_mapping[pred_label[9:]] ] += 1
        
        csv_writer_predictions.writerow( [ sen, label, pred_label, 0 ] )
...
evaluate('test_combine_native_12', 12)
