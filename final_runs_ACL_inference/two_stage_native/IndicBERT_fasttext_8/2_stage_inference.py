...
fasttext_lid_model_path='/nlsasfs/home/ai4bharat/yashkm/yash/indic-lid/final_runs_ACL_inference/native_model/fasttext/clean_samples/tune_run/baseline_en_other/result_model_dim_8/model_baseline_roman.bin'
...
IndicBERT_lid_model_path = '/nlsasfs/home/ai4bharat/yashkm/yash/indic-lid/final_runs_ACL_inference/native_model/finetuning/clean_samples/IndicBERT/unfreeze_layers/result_unfreeze_1/basline_nn_simple.pt'
...
file = open('../result_threshold_'+str(threshold)+'/result_acc.txt', 'w')

def evaluate(file_name, languages):
    classes = 26
    ...
    # save predictions
    # file_predictions = open('../result/predictions_'+file_name+'.csv', 'w')
    # csv_writer_predictions = csv.writer(file_predictions)
    # csv_writer_predictions.writerow( [ 'Sentence', 'Ground truth', 'Prediction', 'Score' ] )

    file_predictions_right = open('../result_threshold_'+str(threshold)+'/right_predictions_'+file_name+'.csv', 'w')
    csv_writer_predictions_right = csv.writer(file_predictions_right)
    csv_writer_predictions_right.writerow( [ 'Sentence', 'Ground truth', 'Prediction', 'Score', 'Model' ] )


    file_predictions_wrong = open('../result_threshold_'+str(threshold)+'/wrong_predictions_'+file_name+'.csv', 'w')
    csv_writer_predictions_wrong = csv.writer(file_predictions_wrong)
    csv_writer_predictions_wrong.writerow( [ 'Sentence', 'Ground truth', 'Prediction', 'Score', 'Model' ] )
            ...
            for sen, label, pred_label, logit in zip(inputs, labels, predicted, outputs.logits):
                ...
                if fasttext_pred_score > threshold:
                    if confusion_matrix_mapping[fasttext_pred_label[9:]] == label.item():
                        final_count+=1
                        csv_writer_predictions_right.writerow( [ sen, confusion_matrix_reverse_mapping[label.item()], fasttext_pred_label[9:], fasttext_pred_score, 'fasttext' ] )
                        confusion_matrix[ label.item() ][ confusion_matrix_mapping[fasttext_pred_label[9:]] ] += 1

                    else:
                        csv_writer_predictions_wrong.writerow( [ sen, confusion_matrix_reverse_mapping[label.item()], fasttext_pred_label[9:], fasttext_pred_score, 'fasttext' ] )
                    ...
                    if label.item() == pred_label.item():
                        final_count+=1
                        csv_writer_predictions_right.writerow( [ sen, confusion_matrix_reverse_mapping[label.item()], confusion_matrix_reverse_mapping[pred_label.item()], pred_score, 'IndicBERT' ] )
                        confusion_matrix[ label.item() ][ pred_label.item() ] += 1

                    else:
                        csv_writer_predictions_wrong.writerow( [ sen, confusion_matrix_reverse_mapping[label.item()], confusion_matrix_reverse_mapping[pred_label.item()], pred_score, 'IndicBERT' ] )
                        csv_writer_predictions_wrong.writerow( [ sen, confusion_matrix_reverse_mapping[label.item()], fasttext_pred_label[9:], fasttext_pred_score, 'fasttext' ] )
                        confusion_matrix[ label.item() ][ pred_label.item() ] += 1
    ...
    file_precision_recall_f1 = open('../result_threshold_'+str(threshold)+'/precision_recall_f1_'+file_name+'.csv', 'w')
    ...
    file_confusion_matrix = open('../result_threshold_'+str(threshold)+'/confusion_matrix_'+file_name+'.csv', 'w')
...
# evaluate( 'test_combine_flores200_romanized', model)
# evaluate( 'test_combine_ai4b_romanized', model)
# evaluate( 'test_scored_dakshina_romanized', model)

evaluate('test_combine_native', 24)
evaluate('test_AI4Bharat_native', 10)
evaluate('test_dakshina_native', 11)
evaluate('test_flores200_native', 19)
