...
for lang_code in lang_code_list:
    ...
    # model = fasttext.load_model('../model_baseline_native.bin')

    model = fasttext.load_model('/nlsasfs/home/ai4bharat/yashkm/yash/indic-lid/fasttext/experiments/native_model/runs/native_all_1lakh_sample_adding_en/result/model_baseline_native.bin')
    ...
    for line in lines_in:
        ...
        pred_score = model.predict(line)[1][0]
        
        if pred_label[9:] == lang_code_dict[lang_code]:
            if pred_score > 0.8:
                lines_out.append(line)
            
        if len(lines_out) == samples:
            print( lang_code_dict[lang_code] + " : " + str(counter) )
            break
